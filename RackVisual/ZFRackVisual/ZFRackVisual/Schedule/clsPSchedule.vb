        ...
        With cPrint
            ...
            Select Case PrintType
                ...
                Case ePrintType.OverviewPDF
                    SheetChartOverview(NDay, frmSch)
                Case ePrintType.AllPageExcel, ePrintType.ZFEmpty, ePrintType.ZFFG, ePrintType.CustomerFG, ePrintType.CustomerEmtpyReturn
                    SheetChartDay(NDay, frmSch, PrintType)
                    SheetChartOverview(NDay, frmSch)
...
ExitFunction:
    ...
    Private Sub SheetChartDay(ByVal nDay As Integer, ByVal frmSch As clsVSchedule, ByVal PrintType As Integer)
        Dim ColInsCusReturn As Integer = IIf(PrintType = ePrintType.AllPageExcel, 27, 8)
        Dim ColInsCusFG As Integer = IIf(PrintType = ePrintType.AllPageExcel, 22, 8)
        Dim ColInsZFFG As Integer = IIf(PrintType = ePrintType.AllPageExcel, 13, 8)
        ...
        With cPrint
            Select Case PrintType
                Case ePrintType.AllPageExcel : .TemplateSheetName = "TPChartDay"
                Case ePrintType.ZFEmpty : .TemplateSheetName = "TPChartDay1"
                Case ePrintType.ZFFG : .TemplateSheetName = "TPChartDay2"
                Case ePrintType.CustomerFG : .TemplateSheetName = "TPChartDay3"
                Case ePrintType.CustomerEmtpyReturn : .TemplateSheetName = "TPChartDay4"
            End Select

            .OutputSheetName = "Chartday"
            .CreateSheet(.TemplateSheetName, .TemplateSheetName, .OutputSheetName)

            If PrintType = ePrintType.AllPageExcel Or PrintType = ePrintType.ZFEmpty Or PrintType = ePrintType.ZFFG Then
            ...
            If PrintType = ePrintType.AllPageExcel Or PrintType = ePrintType.CustomerFG Or PrintType = ePrintType.CustomerEmtpyReturn Then
                Dim strCol As String = IIf(PrintType = ePrintType.AllPageExcel, "Q", "C")
                .setSheatHeader(strCol & "28", Converts.ToInt_NullIsZero(frmSch.txtCapacityZF.Text.Trim))
                .setSheatHeader(strCol & "29", Converts.ToInt_NullIsZero(frmSch.txtBreakTimeZF.Text.Trim))
                .setSheatHeader(strCol & "30", Converts.ToInt_NullIsZero(frmSch.txtWorkingHrsZF.Text.Trim))
                .setSheatHeader(strCol & "31", Converts.ToInt_NullIsZero(frmSch.txtWorkTimeZF.Text.Trim))
                .setSheatHeader(strCol & "32", Converts.ToInt_NullIsZero(frmSch.txtTotalCapacityZF.Text.Trim))
                .setSheatHeader(strCol & "33", Converts.ToInt_NullIsZero(frmSch.txtTrackTimeZF.Text.Trim))
            End If
            ...
            For iLoop As Integer = 10 To 0 Step -1
                Select Case iLoop
                    Case clsCSchedule.eGridType.CusReturn
                        If PrintType = ePrintType.AllPageExcel Or PrintType = ePrintType.CustomerEmtpyReturn Then
                            nRange = GetNRange(frmSch.dtCusEmptyReturn(0)) : ColIns = ColInsCusReturn
                        Else
                            GoTo NextLoop1
                        End If
                    Case clsCSchedule.eGridType.CusFG
                        If PrintType = ePrintType.AllPageExcel Or PrintType = ePrintType.CustomerFG Then
                            nRange = GetNRange(frmSch.dtCusFG(0)) : ColIns = ColInsCusFG
                        Else
                            GoTo NextLoop1
                        End If
                    Case clsCSchedule.eGridType.ZFFG
                        If PrintType = ePrintType.AllPageExcel Or PrintType = ePrintType.ZFFG Then
                            nRange = GetNRange(frmSch.dtZFFG(0)) : ColIns = ColInsZFFG
                        Else
                            GoTo NextLoop1
                        End If
                    Case clsCSchedule.eGridType.ZFEmptyRack
                        If PrintType = ePrintType.AllPageExcel Or PrintType = ePrintType.ZFEmpty Then
                            nRange = GetNRange(frmSch.dtZFEmptyRack(0)) : ColIns = ColInsZFEmpty
                        Else
                            GoTo NextLoop1
                        End If
                ...
                If nRange > 3 Then
                    For iIns As Integer = 4 To nRange
                        ...
                        If PrintType = ePrintType.AllPageExcel Then
...
NextLoop1:
            ...
            For iDay As Integer = 1 To nDay
                ...
                For iLoop As Integer = 0 To 10
                    ...
                    If Not dt Is Nothing Then
                        ...
                        Select Case iLoop
                            Case clsCSchedule.eGridType.ZFEmptyRack
                                If PrintType = ePrintType.AllPageExcel Or PrintType = ePrintType.ZFEmpty Then ExcelColumn += 2
                            Case clsCSchedule.eGridType.ZFFG
                                If PrintType = ePrintType.AllPageExcel Or PrintType = ePrintType.ZFFG Then ExcelColumn += 6
                            Case clsCSchedule.eGridType.CusFG
                                If PrintType = ePrintType.AllPageExcel Or PrintType = ePrintType.CustomerFG Then ExcelColumn += 2
                        End Select
                        Dim ChartName As String = "Chart" & iDay & iLoop
                        .AddChart(.OutputSheetName _
                                  , GetExcelCol(Range1) & ExcelRow & ":" & GetExcelCol(Range2) & ExcelRow & "," & GetExcelCol(Range1) & ExcelRow + 3 & ":" & GetExcelCol(Range2) & ExcelRow + 3 _
                                  , GetExcelCol(Range1) & ExcelRow - 25 _
                                  , 50 + (25 * dt.Columns.Count - 2) _
                                  , 250, "Overview", Excel.XlRowCol.xlRows, ChartName, True, False)
                    End If
                Next
            Next
        ...
        End With
    End Sub
    Private Sub SheetChartDay_BK(ByVal nDay As Integer, ByVal frmSch As clsVSchedule, ByVal PrintType As Integer)
...
NextLoop1:
    ...
    Private Sub SheetChartOverview(ByVal nDay As Integer, ByVal frmSch As clsVSchedule)
        ...
        With cPrint
            ...
            For iDay As Integer = 1 To nDay
                ...
                .setSheatHeader("C" & ExcelRow + 6, Converts.ToInt_NullIsZero(frmSch.txtTotalRack.Text.Trim))

                ExcelCol += 1
            Next
            Dim sCol As String = GetExcelCol(ColStart + nDay - 1)
            .AddChart(.OutputSheetName, "B23:" & sCol & "27", "B2", 200 + ((nDay - 1) * 50), 300, "Overview", Excel.XlRowCol.xlColumns, "ChartOverview", True, True)
        End With
    ...
    Private Sub SheetSummary(ByVal nDay As Integer, ByVal frmSch As clsVSchedule)
        ...
        With cPrint
            ...
            .setSheatHeader("C" & ExcelRow + 3, frmSch.txtTotalRack.Text.Trim)
        End With
    End Sub
#End Region
...
#End Region
    ...
    Public Function GetExcelCol(ByVal iCol As Integer) As String
        '1-26 = "A-Z"
        '702 = "ZZ"
        'ASC("A") = 65
        ...
        ElseIf iCol <= 1378 Then
            ...
            Dim LastString As String = ""
            If Remain = 0 Then LastString = "Z" Else LastString = Chr(Remain + 64)
            Return "A" & Prefix & LastString
        ElseIf iCol <= 2054 Then
            iCol = iCol - 1378
            Dim Remain As Integer = iCol Mod 26
            Dim nMulti As Integer = (iCol - Remain) / 26
            Dim Prefix As String = ""
            ...
            Dim LastString As String = ""
            If Remain = 0 Then LastString = "Z" Else LastString = Chr(Remain + 64)
            Return "B" & Prefix & LastString
        ElseIf iCol <= 2730 Then
            iCol = iCol - 2054
            Dim Remain As Integer = iCol Mod 26
            Dim nMulti As Integer = (iCol - Remain) / 26
            Dim Prefix As String = ""
            ...
            Dim LastString As String = ""
            If Remain = 0 Then LastString = "Z" Else LastString = Chr(Remain + 64)
            Return "C" & Prefix & LastString
        ElseIf iCol <= 3406 Then
            iCol = iCol - 2730
            Dim Remain As Integer = iCol Mod 26
            Dim nMulti As Integer = (iCol - Remain) / 26
            Dim Prefix As String = ""
            ...
            Dim LastString As String = ""
            If Remain = 0 Then LastString = "Z" Else LastString = Chr(Remain + 64)
            Return "D" & Prefix & LastString
        ElseIf iCol <= 4082 Then
            iCol = iCol - 3406
            Dim Remain As Integer = iCol Mod 26
            Dim nMulti As Integer = (iCol - Remain) / 26
            Dim Prefix As String = ""
            ...
            Dim LastString As String = ""
            If Remain = 0 Then LastString = "Z" Else LastString = Chr(Remain + 64)
            Return "E" & Prefix & LastString
        ElseIf iCol <= 4758 Then
            iCol = iCol - 4082
            Dim Remain As Integer = iCol Mod 26
            Dim nMulti As Integer = (iCol - Remain) / 26
            Dim Prefix As String = ""
