...
Public Class clsVTruckBMW
    Private cls As New clsCTruckBMW
...
#Region "Load"
    ...
    Private Sub LoadForm()
        ...
        ComboSet.TextString(txtDriverName, dtLength, "DriverName")
        ComboSet.TextString(txtTel, dtLength, "Tel")
        ComboSet.TextString(txtZFInCharge, dtLength, "ZFInCharge")
        ComboSet.TextString(txtCusInCharge, dtLength, "CusInCharge")
...
#Region "Toolbar Click"
    ...
    Private Sub ClearControlDetail()
        ...
        txtTel.Clear()
        txtZFInCharge.Clear()
        txtCusInCharge.Clear()
        chkAxleCompleted.Checked = False
    End Sub
    ...
    Private Sub DisplayUpdate(ByVal rGrid As DataRow, ByVal RowIndex As Integer)
        ...
        txtModel.Text = Converts.ToStr(rGrid("Model"))
        txtVarian.Text = Converts.ToStr(rGrid("Varian"))
        txtLot.Text = Converts.ToStr(rGrid("Lot"))
        txtDeliveryNoteNo.Text = Converts.ToStr(rGrid("DeliveryNoteNo"))
        txtAxleNo.Text = Converts.ToStr(rGrid("AxleNo"))
        txtDriverName.Text = Converts.ToStr(rGrid("DriverName"))
        txtTel.Text = Converts.ToStr(rGrid("Tel"))
        txtZFInCharge.Text = Converts.ToStr(rGrid("ZFInCharge"))
        txtCusInCharge.Text = Converts.ToStr(rGrid("CusInCharge"))
        chkAxleCompleted.Checked = Converts.ToInt_NullIsZero(rGrid("AxleCompleted"))
    End Sub
#End Region
...
#Region "Data to Grid"
    ...
    Private Sub DataToGrid()
        ...
        drAdd("Varian") = txtVarian.Text.Trim
        drAdd("Lot") = txtLot.Text.Trim
        drAdd("DeliveryNoteNo") = txtDeliveryNoteNo.Text.Trim
        drAdd("AxleNo") = txtAxleNo.Text.Trim
        drAdd("DriverName") = txtDriverName.Text.Trim
        drAdd("Tel") = txtTel.Text.Trim
        drAdd("ZFInCharge") = txtZFInCharge.Text.Trim
        drAdd("CusInCharge") = txtCusInCharge.Text.Trim
        drAdd("AxleCompleted") = IIf(chkAxleCompleted.Checked = False, 0, 1)
