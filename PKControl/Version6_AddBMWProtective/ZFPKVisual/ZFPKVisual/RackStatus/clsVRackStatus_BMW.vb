...
Public Class clsVRackStatus_BMW
    ...
    Private dtMBikeFront As DataTable = Nothing
    Private dtMBikeRear As DataTable = Nothing
...
#Region "Load"
        ...
        With cboSModel
            ...
            .DataSource = DataBase.DataSelect("MstModel" _
                                                  ...
                                                  , "ID " & _
                                                    ...
                                                    ",FrontFlag,RearFlag,FrontRearFlag,STBFlag,MBikeFrontFlag,MBikeRearFlag" & _
    ...
    Private Sub LoadData()
        ...
        dtMBikeFront = dtStatus.Clone
        dtMBikeRear = dtStatus.Clone
        ...
        If dtStatus.Rows.Count > 0 Then
            ...
            For Each dr As DataRow In dtStatus.Select("Type=" & SystemINI.ModelTypeBMW.STB & "", "Code,RackNo")
                dtSTB.ImportRow(dr)
            Next
            For Each dr As DataRow In dtStatus.Select("Type=" & SystemINI.ModelTypeBMW.MBikeFront & "", "Code,RackNo")
                dtMBikeFront.ImportRow(dr)
            Next
            For Each dr As DataRow In dtStatus.Select("Type=" & SystemINI.ModelTypeBMW.MBikeRear & "", "Code,RackNo")
                dtMBikeRear.ImportRow(dr)
            Next
...
#Region " Grid M-Bike Front "
    Private Sub grdMBikeFront_BindingGridColumns() Handles grdMBikeFront.BindingGridColumns
        With grdMBikeFront
            .Columns.Clear()
            .Rows.Clear()
            For i As Integer = 0 To CustomizeSetting.INI.RackColumn - 1
        ...
        End With
    End Sub
    Private Sub grdMBikeFront_CellDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdMBikeFront.CellDoubleClick
        btnEdit.PerformClick()
    End Sub
    Private Sub grdMBikeFront_CellEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdMBikeFront.CellEnter
        Dim RackColumn As Integer = CustomizeSetting.INI.RackColumn
        If e.RowIndex = 0 OrElse (e.RowIndex - 1) Mod 3 = 0 Then
            Dim DTRow As Integer = (((e.RowIndex - 1) / 3) * RackColumn) + e.ColumnIndex
            If DTRow >= 0 AndAlso DTRow <= dtMBikeFront.Rows.Count - 1 Then btnEdit.Enabled = True Else btnEdit.Enabled = False
        Else
            btnEdit.Enabled = False
        End If
    End Sub
#End Region
#Region " Grid M-Bike Rear "
    Private Sub grdMBikeRear_BindingGridColumns() Handles grdMBikeRear.BindingGridColumns
        With grdMBikeRear
            .Columns.Clear()
            .Rows.Clear()
            For i As Integer = 0 To CustomizeSetting.INI.RackColumn - 1
        ...
        End With
    End Sub
    Private Sub grdMBikeRear_CellDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdMBikeRear.CellDoubleClick
        btnEdit.PerformClick()
    End Sub
    Private Sub grdMBikeRear_CellEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdMBikeRear.CellEnter
        Dim RackColumn As Integer = CustomizeSetting.INI.RackColumn
        If e.RowIndex = 0 OrElse (e.RowIndex - 1) Mod 3 = 0 Then
            Dim DTRow As Integer = (((e.RowIndex - 1) / 3) * RackColumn) + e.ColumnIndex
            If DTRow >= 0 AndAlso DTRow <= dtMBikeRear.Rows.Count - 1 Then btnEdit.Enabled = True Else btnEdit.Enabled = False
        Else
            btnEdit.Enabled = False
        End If
    End Sub
...
#End Region
    ...
    Private Sub ShowStatus()
        If dtStatus Is Nothing OrElse dtStatus.Rows.Count = 0 Then
            ...
            txtSTB.Text = "" : txtMBikeFront.Text = "" : txtMBikeRear.Text = "" : txtPTCQty.Text = ""
            Exit Sub
        End If
        ...
        txtMBikeFront.Text = Format(dtMBikeFront.Rows.Count, "#,###")
        txtMBikeRear.Text = Format(dtMBikeRear.Rows.Count, "#,###")
        ...
        End If
        Dim RowMBikeFront As Integer = dtStatus.Compute("count(Type)", "Type=" & SystemINI.ModelTypeBMW.MBikeFront)
        If RowMBikeFront > 0 Then
            If RowMBikeFront Mod RackColumn = 0 Then RowMBikeFront = RowMBikeFront / RackColumn Else RowMBikeFront = (RowMBikeFront - (RowMBikeFront Mod RackColumn)) / RackColumn + 1
        End If
        Dim RowMBikeRear As Integer = dtStatus.Compute("count(Type)", "Type=" & SystemINI.ModelTypeBMW.MBikeRear)
        If RowMBikeRear > 0 Then
            If RowMBikeRear Mod RackColumn = 0 Then RowMBikeRear = RowMBikeRear / RackColumn Else RowMBikeRear = (RowMBikeRear - (RowMBikeRear Mod RackColumn)) / RackColumn + 1
        End If
        ...
        Dim AllRow As Integer = RowFront + RowRear + RowFrontRear + RowSTB + RowMBikeFront + RowMBikeRear
        '=================================

        For iTypeLoop As Integer = 1 To 7
            ...
            Select Case iTypeLoop
                ...
                Case 5 : iType = SystemINI.ModelTypeBMW.MBikeFront
                Case 6 : iType = SystemINI.ModelTypeBMW.MBikeRear
                Case 7 : iType = SystemINI.ModelTypeBMW.PTC
            End Select
            ...
            ElseIf iType = SystemINI.ModelTypeBMW.STB Then
                dtShow = dtSTB
            ElseIf iType = SystemINI.ModelTypeBMW.MBikeFront Then
                dtShow = dtMBikeFront
            ElseIf iType = SystemINI.ModelTypeBMW.MBikeRear Then
                dtShow = dtMBikeRear
            Else
                dtShow = dtPTC
            End If
            ...
            Select Case iType
                ...
                Case SystemINI.ModelTypeBMW.MBikeFront : grdShow = grdMBikeFront
                Case SystemINI.ModelTypeBMW.MBikeRear : grdShow = grdMBikeRear
            ...
            With grdShow
                ...
                Select Case iType
                    ...
                    Case SystemINI.ModelTypeBMW.MBikeFront : RowAdd = RowMBikeFront
                    Case SystemINI.ModelTypeBMW.MBikeRear : RowAdd = RowMBikeRear
    ...
    Private Sub btnEdit_Click(ByVal sender As System.Object, ByVal e As OPE_Controls.EventArgs) Handles btnEdit.Click
        ...
        ElseIf TabControl.SelectedIndex = SystemINI.ModelTypeBMW.STB Then
            grdShow = grdSTB
            dtShow = dtSTB
        ElseIf TabControl.SelectedIndex = SystemINI.ModelTypeBMW.MBikeFront Then
            grdShow = grdMBikeFront
            dtShow = dtMBikeFront
        ElseIf TabControl.SelectedIndex = SystemINI.ModelTypeBMW.MBikeRear Then
            grdShow = grdMBikeRear
            dtShow = dtMBikeRear
        Else
            grdShow = grdPTC
            dtShow = dtPTC
    ...
    Private Sub btnSearch_Click(ByVal sender As Object, ByVal e As OPE_Controls.EventArgs) Handles btnSearch.Click
        ...
        grdMBikeRear_BindingGridColumns()
        ...
        If grdFront.RowCount > 0 Or grdFront.RowCount + grdRear.RowCount + grdFrontRear.RowCount + grdSTB.RowCount + grdMBikeFront.RowCount + grdMBikeRear.RowCount + grdPTC.RowCount = 0 Then
        ...
        ElseIf grdMBikeFront.RowCount > 0 Then
            TabControl.SelectedIndex = SystemINI.ModelTypeBMW.MBikeFront
        ElseIf grdMBikeRear.RowCount > 0 Then
            TabControl.SelectedIndex = SystemINI.ModelTypeBMW.MBikeRear
        Else
