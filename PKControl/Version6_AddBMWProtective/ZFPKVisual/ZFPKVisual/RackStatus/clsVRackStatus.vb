...
#Region "Load"
    ...
    Private Sub LoadForm()
        ...
        With cboSStatus
            ...
            .DataSource = DataBase.DataTable("select '" & CustomizeSetting.INI.Status_ZF & "' as RackStatus,1 as SortOrder " & _
                                             "union select '" & CustomizeSetting.INI.Status_Customer & "' as RackStatus,2 as SortOrder " & _
    ...
    Private Sub LoadData()
        ...
        If dtStatus.Rows.Count > 0 Then
            For Each dr As DataRow In dtStatus.Select("Type=" & SystemINI.ModelTypeMB.Front & "", "Code,RackNo")
                dtFront.ImportRow(dr)
            Next
            For Each dr As DataRow In dtStatus.Select("Type=" & SystemINI.ModelTypeMB.Rear & "", "Code,RackNo")
                dtRear.ImportRow(dr)
            Next
            For Each dr As DataRow In dtStatus.Select("Type=" & SystemINI.ModelTypeMB.BPS & "", "Code,RackNo")
                dtBPS.ImportRow(dr)
            Next
...
#End Region
    ...
    Private Sub ShowStatus()
        ...
        '=================================
        Dim RowFront As Integer = dtStatus.Compute("count(Type)", "Type=" & SystemINI.ModelTypeMB.Front)
        If RowFront > 0 Then
        ...
        End If
        Dim RowRear As Integer = dtStatus.Compute("count(Type)", "Type=" & SystemINI.ModelTypeMB.Rear)
        If RowRear > 0 Then
            If RowRear Mod RackColumn = 0 Then RowRear = RowRear / RackColumn Else RowRear = (RowRear - (RowRear Mod RackColumn)) / RackColumn + 1
        End If
        Dim RowBPS As Integer = dtStatus.Compute("count(Type)", "Type=" & SystemINI.ModelTypeMB.BPS)
        If RowBPS > 0 Then
        ...
        For iTypeLoop As Integer = 1 To 3
            ...
            Select Case iTypeLoop
                Case 1 : iType = SystemINI.ModelTypeMB.Front
                Case 2 : iType = SystemINI.ModelTypeMB.Rear
                Case 3 : iType = SystemINI.ModelTypeMB.BPS
            End Select
            Dim dtShow As DataTable = Nothing 'IIf(iType = SystemINI.ModelType.Front, dtFront, dtRear)
            If iType = SystemINI.ModelTypeMB.Front Then
                dtShow = dtFront
            ElseIf iType = SystemINI.ModelTypeMB.Rear Then
                dtShow = dtRear
            Else
                dtShow = dtBPS
            End If
            ...
            Select Case iType
                Case SystemINI.ModelTypeMB.Front : grdShow = grdFront
                Case SystemINI.ModelTypeMB.Rear : grdShow = grdRear
            ...
            With grdShow
                ...
                Select Case iType
                    Case SystemINI.ModelTypeMB.Front : RowAdd = RowFront
                    Case SystemINI.ModelTypeMB.Rear : RowAdd = RowRear
                ...
                If RowAdd > 0 Then
                    ...
                    For Each drRackStatus As DataRow In dtShow.Select()
                        ...
                        Select Case drRackStatus("RackStatus")
                            ...
                            Case CustomizeSetting.INI.Status_Customer : .Rows(RowData2).Cells("Col" & iCol).Value = My.Resources.StateMB
        ...
        If frm.ShowDialog = Windows.Forms.DialogResult.Yes Then
            ...
            With grdShow.Rows(Row).Cells(Col)
                Select Case RackStatus
                    ...
                    Case CustomizeSetting.INI.Status_Customer : .Value = My.Resources.StateMB
