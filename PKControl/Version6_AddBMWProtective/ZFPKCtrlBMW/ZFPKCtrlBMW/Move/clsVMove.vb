...
#End Region
        ...
        If ScanValidate(Rack, Remark, Model, Code, Running) = True Then
            ...
            If Not dtMstItem Is Nothing Then
                If dtMstItem.Rows.Count > 0 Then
                    ...
                    If strFrom = clsCMove.Status_OnTruck Then strFrom = clsCMove.Status_BMW
                    Dim strTo As String = ""
                    Select Case mScanType
                        Case eScanType.FG
                            strTo = clsCMove.Status_FG
                        Case eScanType.BMW
                            If strFrom = clsCMove.Status_FG Then
                        ...
                        Case eScanType.Ext
                            ...
                            Else
                                strTo = clsCMove.Status_ZF
                            End If
                        Case eScanType.RepainInZF
                            If strFrom = clsCMove.Status_ZF Then
                                strTo = clsCMove.Status_RepairInZF
                            Else
                                strTo = clsCMove.Status_ZF
                            End If
                    End Select
    ...
    Private Function ScanValidate(ByVal Rack As String, ByVal Remark As String, ByVal Model As String, ByVal Code As String, ByVal Running As String) As Boolean
        ...
        '-----------Case นี้เหมือนเดิม ไม่เปลี่ยน-----------
        Dim CurStatus As String = dtMst.Rows(0)("Current").ToString
        If CurStatus = clsCMove.Status_OnTruck Then CurStatus = clsCMove.Status_BMW
        If dtPrgTrans.Rows.Count > 0 Then
        ...
        Select Case mScanType
            Case eScanType.BMW
                If CurStatus = clsCMove.Status_FG Or CurStatus = clsCMove.Status_BMW Then
                    StatusOK = True
                End If
            Case eScanType.Sup
            ...
            Case eScanType.Ext
                ...
                End If
            Case eScanType.FG
                If CurStatus = clsCMove.Status_ZF Then
                    StatusOK = True
                End If
            Case eScanType.RepainInZF
                If CurStatus = clsCMove.Status_ZF Or CurStatus = clsCMove.Status_RepairInZF Then
                    StatusOK = True
                End If
        End Select
        If StatusOK = False Then
            If mScanType = eScanType.BMW And CurStatus = clsCMove.Status_OnTruck Then
                PlayBeeb()
