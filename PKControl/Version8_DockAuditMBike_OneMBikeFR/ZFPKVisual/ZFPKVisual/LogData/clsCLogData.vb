...
Public Class clsCLogData
    ...
    Public Function GetDailyLog(LogDate As Date) As DataTable
        Dim QColModel As String = ""
        If CustomerCode = "MB" Then
            QColModel = "mModel.Model"
        Else
            QColModel = "case when mModel.Model=mModel.ModelContainer then mModel.Model else mModel.Model + ' (' + mModel.ModelContainer + ')' end as Model"
        End If
        Dim sql As String = String.Format( _
            "select RackID," & QColModel & ",RackNo,CONCAT(Code,RackNo) as Show,mRackHD.Remark,[To] as RackStatus {0}" & _
            ...
            "       ,LossFlag,LossDate,LossRemark,License,PrgTruck.CreateDate as ShipDate,PrgTruck.Remark as TruckRemark {0} " & _
            "", vbCrLf)
        If CustomerCode = "BMW" Then
            sql &= String.Format( _
            "       ,PrgTruck.Model as TruckModel,PrgTruck.Varian,PrgTruck.Lot,PrgTruck.DeliveryNoteNo,PrgTruck.AxleNo {0}" & _
            "", vbCrLf)
        End If

        sql &= String.Format("from PrgTransaction Trans {0}" & _
