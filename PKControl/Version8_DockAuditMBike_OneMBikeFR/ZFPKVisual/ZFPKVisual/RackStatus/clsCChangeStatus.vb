...
Public Class clsCChangeStatus
    Public Function GetData(RackDTID As Integer, OnTruckTransfer As String) As DataTable
        Dim sql As String = String.Format( _
            "select Model,ModelContainer,cModelType.Name as Type,Code,Remark,RackNo,RackID,'ZF' as RackStatus,LossFlag {0}" & _
            ...
            "where mRackDT.ID = {1} And isnull(mRackDT.Replaced,0) = 0 {0}" & _
            "union {0}" & _
            "select Model,ModelContainer,cModelType.Name as Type,Code,Remark,RackNo,RackID {0}" & _
            "       ,case when [To]='On Truck' and DATEDIFF(HOUR,ScanDateTime,GetDate())>=5 then '" & OnTruckTransfer & "' else [To] end as RackStatus,LossFlag {0}" & _
            "from MstModel mModel {0}" & _
            "inner join MstRackHD mRackHD on mModel.ID=mRackHD.ModelID {0}" & _
            "inner join MstRackDT mRackDT on mRackHD.RackHeaderID=mRackDT.RackHeaderID {0}" & _
            "inner join CstModelType cModelType on mRackHD.Type=cModelType.ID {0}" & _
    ...
    Public Function Save(RackDTID As Integer, CurStatus As String, NewStatus As String) As Boolean
        ...
        DataBase.BeginTrans()
        Try
            Dim dtRackDT As DataTable = DataBase.DataSelect("MstRackDT", "ID=" & RackDTID & " and isnull(Replaced,0)=0")
            ...
            Dim sql As String = String.Format( _
                ...
                , vbCrLf, ScanType, RackDTID, CurStatus, NewStatus, StaffCode, StaffID)
