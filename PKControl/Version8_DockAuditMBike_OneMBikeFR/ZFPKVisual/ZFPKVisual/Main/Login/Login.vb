...
#Region " Load "
    Public Sub Login_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim InfoProjectPathCus As String = Info.ProjectPath & "\" & CustomerCode
        Messages.LogFile = Info.ProjectPath & "\LogError\"
        Dim oINI As ArrayList = ComboSet.ReadTextFileList(Err, InfoProjectPathCus & "\Config.INI")
        ...
        For Each sINI As String In oINI
            ...
            Select Case aINI(0).Trim.ToLower
                ...
                Case "adbpath" : INI.ADBPath = aINI(1).Trim
                Case "handyinterface" : INI.HandyInterface = aINI(1).Trim
                Case "mstfilename" : INI.MstFilename = aINI(1).Trim
                Case "mstitemfilename" : INI.DockAuditMstItemFileName = aINI(1).Trim
                Case "mstmodeldockfilename" : INI.DockAuditMstModelDockFileName = aINI(1).Trim
                Case "mstpartfilename" : INI.DockAuditMstPartFileName = aINI(1).Trim
                Case "mststafffilename"
                    INI.DockAuditMstStaffFileName = aINI(1).Trim
                    INI.StaffFilename = aINI(1).Trim
                Case "mstqcfilename" : INI.DockAuditMstQCFileName = aINI(1).Trim
                Case "dockauditinterface" : INI.DockAuditInterface = aINI(1).Trim
            End Select
        Next
        oINI = Nothing
        DataBase = Nothing
        DataBase = New OPE_Controls.Util.DBUtil()
        DataBase.DB_Type = OPE_Controls.Util.DBUtil.eDBType.SqlServer
        DataBase.DBName = INI.DBName

        DataBase.ConnString1 = String.Format("Server={0};Database={1};uid={2};pwd={3}", INI.Server, DataBase.DBName, INI.UserName, INI.Password)
        ...
        PicMB.Visible = (CustomerCode = "MB")
        PicBMW.Visible = (CustomerCode = "BMW")
        CustomizeSetting.INI.Status_Customer = CustomerCode

        'txtUserName.Text = "salintip"
        'txtPassword.Text = "18028"
