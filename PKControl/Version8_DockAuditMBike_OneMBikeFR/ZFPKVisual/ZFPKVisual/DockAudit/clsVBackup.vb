...
'    '    Dim sErr1 As String = "Saving Dock Audit" & vbCrLf
'    '    Dim sErr2 As String = "Saving Dock Audit Shock" & vbCrLf
...
'        ElseIf ChkInsertDB = False Then
'            If DoingInsertDB = False Then
'                Dim sErr1 As String = "Saving Dock Audit" & vbCrLf
'                Dim sErr2 As String = "Saving Dock Audit Shock" & vbCrLf
'                If TransToDB(sErr1, sErr2) = True Then
'                    txtStatus.Text = sErr1 & sErr2
'                    ChkInsertDB = True
'                End If
'            End If
'        End If
...
'            If ADBPullFromAndroid(HandyFilePath, PCFilePath) = True Then
...
'    Private Function ADBPullFromAndroid(deviceFilePath As String, localDestinationPath As String) As Boolean
'        Dim ret As Boolean = False
'        Dim adbPath As String = INI.ADBPath ' --> C:\platform-tools\adb.exe
...
'                ret = True
'            Else
'                MsgBox("Error during ADB pull: " & sErr)
'            End If
'        End If
...
'                    Dim rCheck1 As Integer = Lines(9)
'                    Dim rCheck2 As Integer = Lines(10)
'                    Dim rCheck3 As Integer = Lines(11)
'                    Dim rCheck4 As Integer = Lines(12)
'                    Dim rCheck5 As Integer = Lines(13)
'                    Dim rCheck6 As Integer = Lines(14)
'                    Dim rCheck7 As Integer = Lines(15)
'                    Dim rStaffCode As String = Lines(16)
'                    Dim rScanDateTime As String = Lines(17)
'                    Dim rCompleteFlag As Integer = Lines(18)
'                    '=============================
...
'                            "                         , Check1, Check2, Check3, Check4, Check5, Check6, Check7                  {0}" & _
'                            "                         , StaffCode, ScanDateTime, CompleteFlag, UpdateUser, UpdateDate)          {0}" & _
'                            "select " & rRackDTID & "," & rModelDockID & ", '" & rVarian & "','" & rLot & "','" & rAxleNo & "','" & rSide & "','" & rBarcode & "'," & rRev & "," & rAxleShock & " {0}" & _
'                            "       ," & rCheck1 & "," & rCheck2 & "," & rCheck3 & "," & rCheck4 & "," & rCheck5 & "," & rCheck6 & "," & rCheck7 & "                                              {0}" & _
'                            "       ,'" & rStaffCode & "','" & rScanDateTime & "'," & rCompleteFlag & ", 0, getdate()           {0}" _
'                            , vbCrLf)
...
'SaveError:
'        txtStatus.Text = "Save Dock Audit fail."
'        Return False
'    End Function

'    Private Function transferDockAuditShock(sErr As String) As Boolean
'        Try
...
'                    '=============================
'                    Dim dtTrans As DataTable = DataBase.DataSelect("PrgDockAuditShock", "BarcodeHD='" & rBarcodeHD & "'")
'                    If dtTrans.Rows.Count > 0 Then
'                        sErr &= "- Duplicate " & rBarcodeHD & " (Skipped)"
'                    Else
'                        sql = String.Format( _
'                            "insert into PrgDockAuditShock(BarcodeHD, Rev, Barcode, Result, UpdateUser, UpdateDate) {0}" & _
'                            "select '" & rBarcodeHD & "', " & rRev & ", '" & rBarcode & "', '" & rResult & "',0,getdate() {0}" _
'                            , vbCrLf)
'                        If DataBase.runSQL(sql) = False Then GoTo SaveError
'                    End If
'                End If
